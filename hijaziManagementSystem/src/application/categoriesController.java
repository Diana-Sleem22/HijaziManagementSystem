...
public class categoriesController implements Initializable {
	...
	void addCategory(ActionEvent event) throws IOException, ParseException {
		...
		JSONObject response = CommonFunctions.sendHTTPRequest("http://localhost:8080/registerCategory", "POST", "",
				cm.toString());
		int categoryAddedID = Integer
				.parseInt(((JSONObject) ((JSONObject) ((JSONArray) (response.get("data"))).get(0)).get("category"))
						.get("id").toString());
		data.add(new CategoryModel(categoryAddedID, catName.getText().toString()));
	...
	void deleteCategory(ActionEvent event) throws IOException, ParseException {
		...
		CommonFunctions.sendHTTPRequest("http://localhost:8080/deleteCategory", "DELETE", selectedCategory.getId() + "",
				cm.toString());
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
			...
			for (Object element : arr) {
				...
				data.add(new CategoryModel(Integer.parseInt(new_obj.get("id").toString()),
